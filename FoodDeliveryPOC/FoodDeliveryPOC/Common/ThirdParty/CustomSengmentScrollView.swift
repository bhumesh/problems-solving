//
//  CustomSengmentScrollView.swift
//  DinDinnAssignment
//
...
class CustomSengmentScrollView<T: CustomSegmentProtocol>: UIScrollView where T: UIView {
    ...
    //  ThirdPart
    var contentView: CustomContentView!
    ...
    //   Add the third View
    func createContentView() -> CustomContentView {
        let contentView = CustomContentView(frame: CGRect.zero)
...
extension CustomSengmentScrollView {
    func listenContentOffset() {
        ...
        contentViews.forEach { view in
            if let scroll = view as? UIScrollView {
                ...
                // up
                offsetob.observeOn(MainScheduler.asyncInstance).filter { $0.diff > 0.0 }.subscribe { [unowned self] eve in
