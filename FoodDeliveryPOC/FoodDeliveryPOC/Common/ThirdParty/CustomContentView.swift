...
class CustomContentView: UIScrollView {
    var pageIndex = 0
...
extension CustomContentView: UIScrollViewDelegate {
    func scrollViewDidEndDecelerating(_ scrollView: UIScrollView) {
        pageIndex = Int(contentOffset.x / bounds.size.width)
        self.indexChange?(pageIndex)
        
    }
}

extension CustomContentView:CustomSegmentProtocol{
    var change: CustomContentView.didIndexChange {
        get {
            return indexChange!
        }
        set {
            self.indexChange = newValue
        }
    }
